   ...
   FLATTEN (
    EXTRACT(
     ...
     '^(\\S+) (\\S+) (\\S+) \\[([\\w:/]+\\s[+\\-]\\d{4})\\] "(\\S+) (\\S+) \\S+" (\\S+) (\\S+) "([^"]*)" "([^"]*)"'
       ...
       host: chararray, identity: chararray, user: chararray, time: chararray, verb: chararray, url: chararray, request: chararray, status: int, size: chararray, referrer: chararray, agent: chararray
       )
;
